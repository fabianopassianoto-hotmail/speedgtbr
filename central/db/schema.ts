...
export const pilotos = sqliteTable(
  "pilotos",
  {
    ...
    classificacao_gt7: text("classificacao_gt7"),
...
export const formulariosPendentes = sqliteTable(
  "formularios_pendentes",
  {
    ...
    emailAttemptAt: integer("email_attempt_at"),
    revisadoEm: text("revisado_em"),
    destinoTipo: text("destino_tipo"),
    destinoId: text("destino_id"),
  },
  (table) => [
    uniqueIndex("idx_form_submission_key").on(table.submissionKey),
    uniqueIndex("idx_form_pending_phone").on(table.phoneNormalized).where(sql`${table.status} = 'pendente'`),
