...
import { welcomeEmailHtml } from "./registration-email-template";
import logoDataUrl from "../../Logo.jpg?inline";
...
export async function sendRegistrationEmail(db: ReturnType<typeof getD1Binding>, key: string): Promise<string> {
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: config.REGISTRATION_EMAIL_FROM, to: [row.email], reply_to: config.REGISTRATION_EMAIL_REPLY_TO || "speedgtbr@gmail.com", subject: "Seu cadastro na Speed GT Brasil: próximos passos", text: welcomeEmailText(row.nome_completo), html: welcomeEmailHtml(row.nome_completo), attachments: [{ filename: "Logo.jpg", content: logoDataUrl.split(",")[1], content_id: "speedgt-logo", content_type: "image/jpeg" }] }),
