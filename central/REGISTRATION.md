...
O cadastro usa `/cadastro` e o painel administrativo usa `/admin`, sem link no menu público. Os endereços anteriores redirecionam para essas URLs. Os caminhos `/central/api/*` e `/central/assets/*` continuam internos ao aplicativo. O logo do site é servido pelo próprio projeto em `/assets/brand/speed-gt-brasil.png`.
